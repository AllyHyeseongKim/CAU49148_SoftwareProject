	...
	private Friend parserOfFriendListFile(String aLineOfFriendListFile) {
		...
		if (!isCommentLine(aLineRemovedspaceCharacters) && !isEmptyLine(aLineRemovedspaceCharacters)) {
			...
			if (isRightInputInformation(splitedFriendInformationByCategory)) {
				...
				parsedFriendInformation.setGroup(Integer.parseInt(splitedFriendInformationByCategory[1]));
				parsedFriendInformation.setName(splitedFriendInformationByCategory[0]);
				parsedFriendInformation.setPhoneNumber(splitedFriendInformationByCategory[2]);
				parsedFriendInformation.setEmailAddress(splitedFriendInformationByCategory[3]);
				if (splitedFriendInformationByCategory.length == 5) {
					parsedFriendInformation.setPicture(splitedFriendInformationByCategory[4]);
				} else {
